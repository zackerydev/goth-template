...
// CountNamed returns how many contacts have name in the first or last name.
func (store *Store) CountNamed(ctx context.Context, name string) (int, error) {
	pattern := "%" + name + "%"
	var total int
	err := store.db.QueryRowContext(ctx, `
SELECT COUNT(*) FROM contacts
WHERE first_name LIKE ? COLLATE NOCASE
   OR last_name LIKE ? COLLATE NOCASE`, pattern, pattern).Scan(&total)
	if err != nil {
		return 0, fmt.Errorf("count named %q: %w", name, err)
	}
	return total, nil
}
